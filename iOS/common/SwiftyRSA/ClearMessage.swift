...
public class ClearMessage: Message {
    ...
    /// - Throws: SwiftyRSAError
    public func verify(with key: PublicKey, signature: Signature) throws -> Bool {
        ...
        let status = SecKeyRawVerify(key.reference, .PKCS1SHA256, hash, hash.count, signatureBytes, signatureBytes.count)
